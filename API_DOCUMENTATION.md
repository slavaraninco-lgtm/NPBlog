...
5. [Управление несколькими блогами (Multi-Blog) и заголовки изоляции](#-управление-несколькими-блогами-multi-blog-и-заголовки-изоляции)
...
8. [🛠️ Руководство по разработке кастомных клиентов редактора](CUSTOM_CLIENT_GUIDE.md)
...
## 🌐 Управление несколькими блогами (Multi-Blog) и заголовки изоляции

В NPBlog реализована полноценная поддержка одновременного ведения и обслуживания **нескольких независимых блогов**. 

Точно так же, как стандартный веб-редактор, REST API отслеживает и управляет блогами по путям к их директориям, хранящимся в `editor_settings.json`:
- `blog_paths`: список путей к папкам блогов (например `["C:\\xampp\\htdocs\\data", "C:\\xampp\\htdocs\\data_tech"]` или `["/var/www/html/data", "/var/www/html/data_news"]`).
- `active_blog_path`: путь к активному по умолчанию блогу сервера.

Каждая папка блога содержит автономную структуру:
```
data_tech/
├── blog/
│   ├── posts-meta.json    # Метаданные и реестр статей блога
│   ├── post-1.html        # Сгенерированные HTML-файлы
│   └── rss.xml            # Автономная RSS-лента блога
├── uploads/               # Загруженные изображения и вложения
├── drafts/                # Черновики блога
├── templates/             # Шаблоны оформления
├── custom_fonts/          # Пользовательские шрифты
└── blog.html              # Главная страница блога
```

### Две модели взаимодействия для мобильных и внешних клиентов

#### 1. Per-Request изоляция через заголовки или query-параметры (Рекомендуется для мобильных приложений)
Мобильное приложение может в любой момент читать, создавать или изменять статьи в любом настроенном блоге **без изменения глобального состояния сервера** (то есть не влияя на веб-редактор или других администраторов).

Для этого к **любому** запросу API (`/posts`, `/media`, `/drafts`, `/autosaves`, `/backups`, `/templates`, `/settings`) достаточно добавить:
- HTTP-заголовок `X-Blog-Path`: полный путь к блогу (`C:\xampp\htdocs\data_tech` или `/var/www/html/data_tech`)
- *Или* HTTP-заголовок `X-Blog`: название папки (`data_tech`)
- *Или* Query-параметр `?blog=data_tech` или `?blog_path=...`

Пример запроса на публикацию статьи в технологический блог:
```http
POST /api/v1/posts HTTP/1.1
Host: your-blog.com
Authorization: Bearer npb_0123456789abcdef...
X-Blog: data_tech
Content-Type: application/json

{
  "title": "Новость в Tech блог",
  "content": "<p>Текст статьи для data_tech...</p>"
}
```

> [!NOTE]
> Если переданный блог отсутствует в `blog_paths`, API вернет понятную ошибку `404 Not Found` с кодом `blog_not_found` и списком доступных блогов `available_blogs`.

#### 2. Глобальное переключение активного блога
Если вы хотите переключить текущий активный блог сервера (например, при смене блога в интерфейсе настроек), используйте эндпоинт `POST /api/v1/blogs/switch`. Это обновит `active_blog_path` в `editor_settings.json` и активную сессию PHP точно так же, как переключатель блогов в веб-редакторе.
...
#### `GET /v1/blogs` — Список настроенных блогов
- **Авторизация**: `Bearer <token>`
- **Описание**: Возвращает список всех зарегистрированных путей к блогам из `editor_settings.json` с метаданными.
- **Пример ответа (`200 OK`)**:
  ```json
  {
    "success": true,
    "status": 200,
    "message": "Список настроенных блогов успешно получен",
    "data": [
      {
        "id": "4a7d1ed8203c94d97d0ffacfa8c71dec",
        "path": "C:\\xampp\\htdocs\\data",
        "normalized_path": "C:/xampp/htdocs/data",
        "name": "data",
        "folder_name": "data",
        "is_active": true,
        "is_default": true,
      ...
      {
        "id": "7b8c2ee9314d85e08e1ffbcfa9d82efa",
        "path": "C:\\xampp\\htdocs\\data_tech",
        "normalized_path": "C:/xampp/htdocs/data_tech",
        "name": "data_tech",
        "folder_name": "data_tech",
        "is_active": false,
        "is_default": false,
        "exists": true,
        "is_writable": true,
        "posts_count": 5,
        "drafts_count": 0,
        "url": "/data_tech/blog.html"
      }
    ],
    "meta": {
      "total": 2,
      "active_blog": "C:\\xampp\\htdocs\\data"
    }
  }
  ```

#### `GET /v1/blogs/active` — Получение активного блога
- **Авторизация**: `Bearer <token>`
- **Описание**: Возвращает подробную информацию о текущем активном блоге в контексте запроса (с учетом переданных заголовков `X-Blog-Path` / `X-Blog` или глобальной настройки `active_blog_path`).

#### `POST /v1/blogs/switch` — Глобальное переключение активного блога
- **Авторизация**: `Bearer <token>`
- **Тело запроса**:
  ```json
  {
  ...
  ```
  *(Также в параметре `path` или `blog` можно передать просто имя папки, например `"data_tech"`)*
- **Действия**: Сохраняет `active_blog_path` в `editor_settings.json` и в сессии PHP сервера, автоматически перегенерирует RSS-ленту для переключенного блога и возвращает обновленные данные блога и ссылку `blogUrl`.

#### `POST /v1/blogs` — Добавление нового пути к блогу
- **Авторизация**: `Bearer <token>`
- **Тело запроса**:
  ```json
  {
  ...
  ```
- **Параметры**:
  - `path` (string, обязательно) — абсолютный путь на сервере или относительный от корня NPBlog.
  - `make_active` (bool, по умолчанию `false`) — сразу переключить сервер на этот блог.
  - `initialize` (bool, по умолчанию `true`) — автоматически создать необходимую структуру каталогов (`blog/`, `uploads/`, `drafts/`, `templates/`, `custom_fonts/`, `posts-meta.json`), если папка еще не инициализирована.
- **Ответ (`201 Created`)**: Метаданные добавленного блога.

#### `DELETE /v1/blogs` — Удаление пути блога из конфигурации
- **Авторизация**: `Bearer <token>`
- **Тело запроса или Query-параметр**: `{"path": "data_mobile"}` или `DELETE /v1/blogs?path=data_mobile`
- **Безопасность**: Файлы блога на диске **НЕ удаляются**. Путь исключается только из массива `blog_paths` в `editor_settings.json`.
- **Ограничения**: Нельзя удалить единственный оставшийся блог (возвращает `400 Bad Request` с кодом `cannot_delete_last_blog`). Если удалялся текущий активный блог, активным автоматически назначается первый оставшийся блог.
...
#### `DELETE /v1/posts/{id}` (также `DELETE /v1/posts`, `POST /v1/posts/{id}/delete`, `POST /v1/posts/delete`) — Удаление статьи
- **Параметры**:
  - `id` (в пути URL `/v1/posts/{id}`, в строке запроса `?id=1` или в теле JSON `{"id": 1}`).
  - `renumber` (bool, по умолчанию `false`) — если `true`, после удаления автоматически перенумерует оставшиеся статьи по порядку.
- **Действия на сервере**:
  - **Кросс-поиск блога**: если заголовок `X-Blog` не передан, а статья отсутствует в текущем блоге по умолчанию, сервер автоматически ищет статью по всем настроенным блогам (`blog_paths`) и удаляет её именно из того блога, где она находится.
  - Полностью удаляет файл статьи `post-{id}.html` с диска.
  - Удаляет файл фонового изображения `backgrounds/bg-{id}.*` и настройки фона.
  - Переименовывает папку с резервными копиями статьи в `deleted_{id}_{timestamp}` и помечает статус в `backup-meta.json`.
  - Удаляет файл автосохранения `autosave_{id}.json` (если был).
  - Удаляет запись статьи из реестра `posts-meta.json`.
  - Автоматически перегенерирует RSS-ленту блога.
- **Ответ**:
  ```json
  {
    "success": true,
...
### 5. Резервные копии и версионирование
...
### 6. Медиафайлы, фоны, смайлы, шрифты (`/media`) и интеграция со сторонними редакторами
...
## 7. 💻 Готовые примеры кода для мобильных платформ
